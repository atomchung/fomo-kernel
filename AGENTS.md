...
2. A trade the user is deciding on gets `consider`'s computed consequence, and the case for and against is built from that output — never from prose. Mark every claim you add as your own judgment, cite an engine fact through the anchor `references/trade-consequence.md` documents, and name what nobody checked. A case that misquotes the record, drops an owed disclosure, or relabels the user's own words as an outside source is refused, not stored. Leave the decision to the user. Price targets and market forecasts stay out, and a review card's prescription still never names what to buy or sell.
