...
> 人類使用說明見 [README.md](README.md)；**完整流程權威見 [skills/trade-review/SKILL.md](skills/trade-review/SKILL.md)**。
...
1. **先讀完整流程**：`skills/trade-review/SKILL.md` 是四步流程的權威，照它走。
2. **跑 engine（機械層）**：
   ```bash
   ...
   ```
   依賴：Python 3.11+、`yfinance`、`pandas`（見 `requirements.txt`）。沒網路時 engine 會自動退成行為層診斷（α/β 標 note），不會中斷。
...
4. **出卡（收斂層）**：engine 輸出 + 使用者動機答案 → 收斂成一張卡（版型見 `skills/trade-review/card-template.html`）。
