...
#
# Turns the docs/maintainer-guide.md rule "改 engine ... 測試沒全過就不要 commit" from a habit
# into a MECHANISM:
...
reason="$(printf '⛔ commit blocked — tests/run_all.py 未全綠(docs/maintainer-guide.md:測試沒全過就不要 commit)。修好再 commit。\n\n%s' "$fail_tail")"
