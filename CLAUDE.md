...
| HTML card design system | `card-template.html` (design-provenance reference) ↔ the `_HTML_SHIM_CSS` / `_HTML_WIDGET_CSS` literals in `card_renderer.render_html()`. Palette, dark-mode rules, and layout constraints must match; `tests/test_card_html.py::test_widget_fragment_css_stays_mirrored_with_card_template` gates every `.rc`-prefixed rule declaration-for-declaration, both directions (a short, documented exception list covers a few template-only illustrative icons and one accessibility idea the runtime does not implement yet), while `card-template.html` documents the intent. Before adding or reshaping a visual element, read [docs/design-guidelines.md](docs/design-guidelines.md): it defines the module slot contracts that keep one element's change from forcing its neighbours to be re-tuned. |
