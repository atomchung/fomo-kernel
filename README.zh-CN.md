...
|---|---|---|
| **“我该买、加仓、减仓，还是先不动？”** | 计划动作、当前理由，以及现在发生了什么变化 | 已有记录持仓时：精确的交易后权重、集中度／驱动重叠、现金影响、规则冲突、一个主要决策张力与真正的反驳。 |
...
回答会先说明一个有证据的主要决策张力，再给出最强反方，并把限制放在它真正限制的主张旁边。最后动作仍由你决定。
...
图片展示的是复盘 route。交易前回答默认保持简短文字，除非你明确要求更多。
...
- Windows 可以运行不写入 canonical state 的 prepare／preview 路径；但 durable `finalize` 目前会在改动任何 canonical state 前 fail closed，因为实现依赖 POSIX locking 与 directory `fsync`。
