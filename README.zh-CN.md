...
| **同一个决策，但还没有记录持仓** | 决策、理由与为什么是现在 | 不会直接拒绝，而是给出一个有边界的决策框架：最强正方、最强反方、真正决定这笔交易的关键问题，以及哪些组合事实尚未检查。没有虚构数字，默认也不持久化。 |
...
复盘会先把完整卡片显示在对话中，再请你选择规则。文件生成不等于交付；结果必须真正到达你面前。
...
对话仍会继续，但不会假装知道权重、集中度、现金或规则冲突。它只询问会改变判断的少数问题，并说明下一份证据能换来什么更具体的答案。这条没有记录持仓的路径，默认不持久化。
