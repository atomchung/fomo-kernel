...
The third basis, `"recorded_book"`, is what makes a position survive its own book being re-declared (#539/#536). A declaration says what is held and never since when, so an adopted position whose start it did not restate falls back to that declaration's `as_of` — reminting `cycle_id` and orphaning every memory keyed to it, the user's thesis first. `book_refresh.carry_recorded_starts` stamps each still-held position with the start the record already holds, and it is the single primitive both adoption lanes reach: `build_adoption` for the refresh lane, `review._carried_declaration` for the review lane, which is also the only path through which engine-assigned provenance may enter a book at all. Which start is carried, and what the stamp then claims about it, are decided together: an answered estimate stays the user's (`user_estimate`) while the record still traces the position to its anchor, and everything else takes the start the *derived* book states, stamped `recorded_book` — a bookkeeping date for a declared holding, an exact ledger-proven one for a position the record watched open. Reading it from the derived book is what keeps a sold-and-rebought position honest without a further rule: its derived start *is* the rebuy, so there is no previous cycle to inherit. A carried start equal to or later than the declaration's own `as_of` is not written — equal restates the default and would change the anchor's content address without changing a fact, later means the declaration predates the record and the lane that owns that refusal states it plainly.
