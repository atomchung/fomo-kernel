...
**時點語意（釘死，實作照此）**：snapshot 代表 `as_of` 日**收盤後**的狀態——`date == as_of` 的交易視為已反映在宣告數字內、不疊加；嚴格 `date > as_of` 才疊加。用戶宣告「這是我現在的持倉」時 `as_of` = 今天。
...
> 先前提案「只對主市場算＋明標範圍」被否決（2026-07-06）：組合跨市場接近對半時，「主市場」根本不存在，只算一邊等於丟掉一半部位。改為 **per-market 分算**。
...
每個 PR：過六套測試（`python3 tests/run_all.py`）＋ mock fixture 只用假資料＋ **影響用戶可見行為者同 commit 更新 SKILL.md**（契約同步鐵律）。
