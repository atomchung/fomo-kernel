...
The same proven-continuity branch also carries a positive engine-known `add_count` (#660). It is not a broker or agent fact, and it is never inferred from ticker identity: a new snapshot row may contain it only after `carry_recorded_starts` has established the same active cycle. `ledger.derive_holdings` remains the one place that derives `decision_cursor` from that count and the carried cycle id. A sale/re-entry derives a new cycle first, so its count remains zero; a later canonical BUY increments the carried count normally. The field is omitted at zero so legacy anchors remain byte-compatible until a later proven-continuous adoption needs the carry.
