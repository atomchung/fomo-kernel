...
Known gap: `session.py`'s snapshot/reconciliation/adjustment projection (`_project_snapshot_anchor`, two `ledger.append_events` calls) does not yet pass `recorded_at`, so those ledger rows currently land with the field absent rather than the review period's `date_end` an `_ingest_trades`-written row gets. This is an honest gap (unknown, not a guessed value) rather than a wrong one, left for a follow-up once that file is free to edit.
