...
| 7. Findings | `verify --require-findings` fails without exactly one `findings_recorded`, one recorded after the owner verdict, an unrecognized disposition, any field beyond the dispositions, or an `episode:EP-NNN` id absent from `evals/episodes/` — a conversion claim with nothing behind it. Resolved on the write path and again on `verify`, so an edited receipt cannot carry one | judging honestly what counts as a miss, and converting it while the wording is still in front of you |
