...
| 2026-07-23 | axis 2 / layout system follow-up | **Corrects R5 from the entry below.** That rule read "decorative graphics are block-scoped, not indicator-scoped" and moved the P&L curve out of the KPI grid into a strip of its own. Owner review rejected the result twice: first the strip was orphaned from the figure it plots, then the hero band that fixed the association cost more space than the curve is worth — the owner's words were that the curve is not valuable enough to justify reserving that many modules for it. The diagnosis behind R5 was wrong. A chart inside a cell was never the defect — the defect was that **one cell carried five parts (label, value, sub, chart, caption) where its neighbours carried three**, and grid rows stretch to their tallest cell. R5 is restated as: *every cell in the metric row has the same three-part shape — label, one body slot, one sub line* — and the body slot may hold a line instead of a number. The curve is therefore one more cell in the grid, sitting directly after the P&L metric it traces, so the row states where the period ended and that cell states how it got there. Owner framing: it shows the volatility of the process rather than a single time-slice point. Cell counts stay data-driven: two lit metrics + the curve = 3 columns; four + the curve = 5 cells wrapping to two rows of three with the curve spanning two, which fills both rows exactly (1+2, then 3). Five equal columns were measured and rejected — at 120px each, English sub lines clipped. Measured: every cell 93px within its row, per-row spread 0, rows 97–99% filled at 760px, zero clipped subs, both locales. `test_every_kpi_cell_has_the_same_three_part_shape` replaces `test_kpi_tile_never_hosts_the_sparkline` and asserts the curve cell has no `.val` of its own and follows the P&L cell. The `.hero`/`.kpirow` band introduced hours earlier is removed entirely. Open follow-up: [#359](https://github.com/atomchung/fomo-kernel/issues/359) — the curve still only presents, it does not judge; owner suggested folding a judgment sentence into this cell. |
