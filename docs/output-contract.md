...
| 2026-07-23 | [#362](https://github.com/atomchung/fomo-kernel/issues/362) + [#363](https://github.com/atomchung/fomo-kernel/issues/363) | Owner ruling: **one concept, one indicator** — and the return indicators are exactly four: absolute money made or lost, cumulative return, annualized return, and excess return. `/fomo-qa` dogfood found the vs-market sentence still printing in full beside the excess tile that already showed its `+N pp` value and `β` sub. Investigating it surfaced the larger defect: the sentence opened with two absolute total returns (`port_tot`, `spy_tot`), and **`port_tot` is the same concept as the card's cumulative return** (`acct_perf.hold_twr`) computed by a different pipeline — the alpha/beta regression's aligned day set drops days either side lacks data for and days the portfolio moved more than ±50%, so the card answered "what did you make?" with two different numbers (703% and 684% on the dogfood persona) and gave the reader no way to tell why. `port_tot`/`spy_tot` are regression intermediates that leaked onto the card; they are internal again. The excess they feed is a genuinely different concept and stays, with its allocation/selection split as its explanation. What the sentence still states is exactly the excess tile's value and sub, so it takes the pnl/payoff treatment: `kpi_id="excess"` with an empty `html_text` — HTML drops it whole, Markdown keeps it as its only carrier. `check_card.py`'s S-2 needle tracks the new sentence shape in both locales; it reads the Markdown card, which is why the removal does not blind it. Known consequence, accepted: the cumulative return and the excess are measured on different day sets, so subtracting one from the other does not yield the benchmark's return — the card prints no benchmark absolute, so the subtraction is not invited. Printing one would require unifying the basis first, which would break the split's identity (allocation + selection = excess). Still open in #363 at the time: the remaining Block-1 items (the "time-weighted return" wording, the alpha interval's placement, the account-gate sentence's tone, the bullet pass). **2026-07-24 correction:** this row's closing clause also folded a second claim into the "one concept, one indicator" principle above — that cumulative `+N pp` (excess) and annualized α were two indicators for the same concept, so this ruling forbade keeping both. **Owner ruling 2026-07-24 reverses that specific extension: both indicators stay.** They are not one concept measured twice: relative-to-market (`+N pp`) is raw, unadjusted, cumulative excess over the whole window; annualized α is β-adjusted, annualized, and carries its own 95% confidence interval. A portfolio at β 2.05 can beat SPY by +261 pp while α sits near zero — there the entire excess is explained by market exposure (leverage), and only α tells the reader whether any of it was skill; cutting either one removes the answer to a question the other cannot answer. α additionally carries recognition value as the market's most common, mainstream framing for risk-adjusted skill. The "one concept, one indicator" principle above still holds for what it was actually about — `port_tot`/`spy_tot` duplicating the card's own cumulative return — see the 2026-07-24 row at the end of this table. |
| 2026-07-23 | axis 2 / layout system ([#361](https://github.com/atomchung/fomo-kernel/pull/361)) | Owner ruling: the card's layout rules become a system instead of a case log. Root cause: colour was the only tokenized axis, so every layout ruling had to name a pixel rather than a scale — 16 distinct spacing values and 7 type steps accumulated, and each ruling bound itself to one symptom instead of the mechanism. Measured proof that this recurs: the "metadata at the top" ruling removed the review window from a KPI tile, but the same defect returned through a different trigger — the sparkline plus its caption held one tile at 209px and grid row-stretch padded all four to match; separately a month-gated review lights only 2 tiles while `.grid4` still declared `repeat(4,1fr)`, leaving 332px of a 634px row empty. Six rules replace the case log, each stated as *what determines this* and, where possible, machine-checked: **R1** one value is expressed once per card (the attribution headline no longer restates the excess KPI tile; on a card with no excess tile the headline stays, since nothing else carries it); **R2** every cell in the metric row keeps the same three-part shape — label, one body slot, one sub capped at two lines — so the tallest cell is bounded rather than one field being banned; **R3** the KPI column count equals the number of cells that lit up (`data-n`), never a fixed four; **R4** exactly one L1 per card and it is Block 4, which carries its own ground (`.sec.keystep`) — the product promises one action, so nothing may outrank it; **R5** a chart is welcome in a cell as long as it keeps the cell's shape: the period curve renders as one more cell whose line takes the value slot and whose caption takes the sub, directly after the P&L metric it traces (two interim treatments — a standalone strip, then a hero band — were rejected by owner review; the defect was never "a chart in a tile" but one cell carrying five parts where its neighbours carry three); **R6** a note attaches to the scope it explains. Layout tokens `--rc-sp-*` (6 steps), `--rc-tx-*` (6 steps: 13px and 19px retired, 17px named for the L1 rule), `--rc-r-*` (3 steps) land in `_HTML_WIDGET_CSS` and mirror into `card-template.html`. Measured after: tiles 209px → 109px at 760px and 93px at 520px, same-row spread 0, zero clipped subs, no horizontal overflow, both locales. Enforced by `test_layout_uses_the_token_scales_not_ad_hoc_pixels` (spacing/type declarations may not hardcode pixels — catches drift even when both stylesheets drift together), `test_every_kpi_cell_has_the_same_three_part_shape` (the curve cell has no `.val` of its own and follows the P&L cell), and `test_next_step_is_the_cards_only_emphasis_ground`; R6 stays manual. Markdown is untouched: R2/R3/R5 are HTML-only (no tile grid exists there), R1 and R6 hold on both surfaces. Open follow-up: [#359](https://github.com/atomchung/fomo-kernel/issues/359) — the curve presents but does not judge. **2026-07-24 correction:** "zero clipped subs... both locales" held at the two widths measured here (520/760px) and did not generalize below them — [#382](https://github.com/atomchung/fomo-kernel/issues/382) found the pnl and payoff tiles' subs clipping at 320-390px at realistic magnitudes; see the 2026-07-24 #382 row below for the fix and the corrected measurement. |
| 2026-07-23 | [#375](https://github.com/atomchung/fomo-kernel/issues/375) | **A gated account level names its own blocker (§4), and a missing broker `Amount` column stops being fatal.** Real-data `/fomo-qa` supplied a cash anchor and still got no account-level TWR/IRR: the source had no `Amount` column, so `load_cash_flows()` read every trade as leaving cash untouched. Root cause ran deeper than one broker's export — `SKILL.md`'s normalized schema never asked the agent to carry `Amount` at all, while `data-contract.md` simultaneously claimed the engine rebuilds cash history from "the per-trade `Amount` footprint already in the source"; an agent following the contract exactly produced a file that could never unlock the account tier, and all 13 mock personas shared one Amount-bearing broker format, so the persona sweep structurally could not see it. Three changes: the schema now carries `Amount` when the source has it; with no such column the engine estimates each footprint as quantity times price — the same price basis `F_H` has always used for the holdings pillar (`perf.py` header), so the asymmetry was the defect — and discloses it; and `account_perf()` returns a structured `gate={status, data}` (plus advisory `notes`) instead of hardcoded zh sentences, which the renderer resolves through the new `account_gate` copy group. The card used to print "locked until cash has a complete anchor" for all seven blockers, telling a user who had already supplied the anchor to go do it again; the second hardcoded site (`block_missing.annualized`, reached when no holdings pillar renders) is fixed the same way via `ANNUALIZED_GAP_NOTE_BY_GATE`. Both degrade to the old generic sentence for legacy bundles and for statuses added later, so a new engine gate can never render blank. The disclosure that matters most is not the excluded commissions: an export whose rows are all trades carries no deposit or withdrawal record, so the rollback turns money paid in later into a starting balance the user never held — measured at −33pp of account TWR on a synthetic three-year account — and `acct_perf_basis` now carries that assumption with its own magnitude (`implied_start_cash_share`), which also gates it so accounts funded up front get no pointless caveat. New `sample_bare_export.csv` puts the schema in the sweep. |
