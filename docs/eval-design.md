...
tests/copy_corpus.py          card copy on the branches no persona reaches,
                              rendered on all three delivery surfaces and
                              compared against a generated golden; also fails
                              when a claimed copy key reaches no surface
...
- Emits an engine-owned `question_opportunity` only for `add_thesis`, `headline_motive`, `initial_thesis`, and `exit_consistency`. Its canonical choices, order, payload requirements, required status, identity, queue rank, and single-clarification budget remain deterministic; due revisits, rule breaches, and recent exits stay engine-rendered.
