...
> **實作現況**：judge 本體 + mutation 驗活已落地，見 [`tests/agent/`](../tests/agent/)（`judge_narrative.py` + `run_judge_eval.py` + `fixtures/`）。目前吃的是人工/既有卡片文字，還沒接上 §2 headless 產卡的 harness——那部分（`cases/*.yaml`、`personas.md`、`run_case.sh`、`check_card.py`、`check_state.py`）仍待辦，見該目錄 README「這裡沒做的」。
