...
- **session 實證(更硬的理由)**:回顧整個 trade-review 的開發,照出的每一個真洞(假分散 92%、winner_early、攤平 142 次、β 1.72、押對賽道 vs 選股)**全是「對事」算出來的,從沒用過「先分型」**。分型是一個我們從沒走過、也不需要的中間層。
...
1. ✅ 標的層診斷已做:按金額排序(小倉不糾結)、多標籤、outcome-bias-aware(「凹單僥倖」不因結果賺就說加對了)、`thesis_q`(從行為推測持股假設讓用戶確認)。
2. ⏳ 加 `revenge_trade`(連敗後加碼)、強化 `overtrading`。
3. ⏳ **卡片 HTML 版型優化**(2026-06-14 owner todo):現在 `show_widget` 產的完整卡視覺醜,之後優化美觀。**流程已定**:確認(持股假設/動機)在**出卡前的對話**完成(SKILL Step 2),卡是**定論不帶問號**(Step 3)——不做成卡上按鈕。
