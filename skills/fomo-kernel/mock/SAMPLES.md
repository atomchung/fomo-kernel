...
## 兩種跑法:模擬真實用戶 vs demo 展示(別搞混——2026-07-04 真人反饋)

engine 判 `is_demo` 的規則 = **輸入路徑含 `mock`**。同一份 sample,兩種用途走不同路徑:

- **persona 模擬(測「真實用戶會看到什麼」)**:把 CSV + driver map **複製到不含 mock 的路徑**再跑(真用戶的對帳單本來就不在 mock 目錄)→ `is_demo=false`,卡面 = 真實形態——**無 demo 標記、無「這是測試」聲明**。測試元信息(狀態隔離到哪、這是模擬)只留在**對話層**跟作者講,一個字都不准上卡:卡上出現作者視角 = 模擬穿幫,測不到真實體驗。
- **demo 展示(真用戶沒給資料,skill 跑 mock 給他看示範)**:直接走 `mock/` 路徑 → `is_demo=true`,卡頭**必**標 `[demo · 非真實成績]`(A-4 斷言不變——這個標記是給「看別人資料的真用戶」防誤會的,不是給測試者的)。
...
- **避開拆股/退市的失真標的**:早期版本用過 SMCI(2024 拆股)、WBA(2024 退市)會讓「套牢 -96%」「404」這種假訊號污染診斷,已換成 AMAT/MRVL、CVS。**NVDA(day_trader/momentum 兩組)是例外保留**:AI 分散度/主題暴險敘事需要它,拆股前(2024-06-10 之前)交易改填當時真實名目價位(如 $830-950 區間),讓 `adjust_for_splits()` 事後調整算對,而非誤填今日拆股後等值價造成雙重縮放(issue #93)。
