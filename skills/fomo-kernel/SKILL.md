...
- **`pnl_curve`**(#167:總損益從一個點延伸成一張圖):復盤期間累積損益曲線,`{points:[{date,cum_ret}...]}`(起點 `cum_ret` 恆 0、終點對齊 `overview.total_pnl` 那個數)或 `{note:...}`(無價格/樣本不足/混市場尚未支援 → 誠實跳過)。**只在 widget 模式畫成 sparkline**(單色細線,不逐點染色,別重回「多色格子熱力圖」);純文字卡沒有視覺化,`note` 不必轉成一句文字硬補——畫法規格見 card-spec。
...
# #166:sid 從 state 內容算(非 time.time()),同 session 重試 no-op、內容衝突 fail closed。
python3 - <<'PY'
