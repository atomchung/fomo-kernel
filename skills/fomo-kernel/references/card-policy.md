...
Block 1's KPI tiles and its prose never restate the same figure twice (#344): the absolute-P&L, payoff-ratio, and annualized-alpha sentences render only where no tile already carries that number (HTML omits them once the matching tile exists; Markdown has no tile grid, so it keeps the full sentence as its only carrier); the alpha sentence keeps its 95% interval and plain-language caveat either way, since a tile cannot hold those. The review-window span and the SPY/VIX backdrop fold into the "vs market" tile's sub line once that tile renders, instead of standing as their own sentence — a card with no such tile this period keeps the standalone line, since nothing else would carry it. An ETF classification sentence explains a rule ("allocation ETFs are excluded from concentration: TICKER X%"), not a fact about the user's own trading, so it renders beside the other Block-2 ETF disclosures rather than as its own fact bullet (#315).
