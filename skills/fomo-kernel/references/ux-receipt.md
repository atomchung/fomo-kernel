...
A converted id is resolved against the bank in this checkout — on the write path *and* again on `verify`, so a hand-authored or later-edited receipt cannot carry a conversion that never happened. Resolution reads each episode file's declared `id`, so a file that does not parse backs no claim. It proves an episode with that id exists, not that the episode is a good one; that is what `evals/run_episodes.py` is for. The event also rejects any field beyond the dispositions, because this is the row a maintainer is most tempted to paste miss text into and it sits inside the state directory's trust boundary.
