...
Record it once, before the owner verdict. Each `--finding` is one miss and its disposition, and there are exactly two honest ones: `episode:EP-NNN` for a miss converted into a replayable episode ([evals/episodes/README.md](../../../evals/episodes/README.md)), or `not-episodable:#NN:<why it cannot be replayed>` for one that only an issue can hold — whether the card reached the screen, for instance, is a receipt question, not an answer question. A converted id is resolved against the bank in this checkout, so recording a conversion that did not happen fails rather than reads as done.
