...
`--challenge-check-file` points at a transient JSON that never enters the trace, the same nature as the grounding check file. It pairs the `challenge` block **from the `consider` call's own stdout** with the exact answer text shown, and — since #767 — that same call's `sector_display` block:

```json
{
  "challenge": {"...": "the emitted block, verbatim — all five keys, uncut"},
  "presented_text": "the exact answer text you showed the user",
  "sector_display": {"...": "the sector_display block from that same stdout, verbatim -- {} when the response named no largest sector"},
  "disclosures_display": {"...": "the disclosures_display block from that same stdout (#739), when the response carried one -- a real sibling field, not yet read or checked by this tool (see below)"}
}
```
...
**Sector fidelity (#767).** `max_sector` (`consequence.*.max_sector`) is the engine's own internal label — a zh literal from `trade_recap.SECTOR_MAP` — and `sector_display` is what `card_renderer.localized_sector` turns it into for the caller's own language; naming the sector from the raw label instead is exactly the leak #746 fixed. The check is deliberately **not** "an English answer must not contain a Chinese sector literal" — that would be language-dependent, and the module this tool lives in states why the rest of its checks are safe only because they are not. Instead: any canonical sector literal (a key of `card_renderer.SECTOR_ID_BY_LEGACY_LABEL`) found in `presented_text` that is **not** one of `sector_display`'s own values is a mismatch (`sector_display_verbatim`, `sector_display_expected`). Nothing about the comparison reads a caller-declared language — a `zh-TW` answer whose `sector_display` legitimately *is* that literal passes, the identical literal in an `en` answer whose `sector_display` says `"semiconductors"` fails, and a user-supplied driver-map category (`localized_sector`'s own passthrough for a label the engine does not recognize) can never match a canonical literal in any language. `sector_display` is **required, not optional**, in the check file whenever the response carried one: an omitted key is refused rather than silently read as "nothing to check", the same write-never-read shape `challenge`'s own five-key contract already refuses to shrink (#429, #480). A response that named no largest sector still states so explicitly with `"sector_display": {}`.
