...
def _event_id(e):
    return (e.get("key"), e.get("week"), e.get("ticker"))


def append_book(book_path, new_events, mark=None):
    ...
    seen_marks = {m.get("week") for m in marks}
    out = []
    for e in new_events or []:
        ...
        out.append(row)
    n_mark = 0
    if mark and mark.get("week") and mark["week"] not in seen_marks:
        out.append({"type": "review_mark", "week": mark["week"],
                    "opportunities": mark.get("opportunities") or {}})
        n_mark = 1
    if out:
...
def main(argv=None):
    ...
    if a.cmd == "append":
        ...
        if not isinstance(events, list):
            ...
            return 1
        n_ev, n_mark = append_book(a.book, events, mark)
