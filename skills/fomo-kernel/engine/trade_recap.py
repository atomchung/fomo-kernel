...
def _cycle_walk(rows):
    ...
    每個當前持倉 ticker 的 position cycle 起始日、序號(第幾次建倉)與加碼次數。
    與 positions() 同累加邏輯(sell 只在有倉時減 → 不跌負),所以 cycle 判定跟持倉一致
    (雙審 gemini#1:修負數誤判 + codex#4:單一 ledger)。序號讓同 ticker 清倉後重建不撞
    id(雙審 codex#3)。CSV 缺期初持倉 → 該 ticker 不在 `start` 裡,呼叫端標 #unknown。

    #814:序號按 **canonical** ticker 計,拼法也一律 canonical。這條車道與帳本車道
    (`ledger.derive_holdings`)因此對同一個 cycle 鑄出同一個 id——先前 CSV 這側用
    存下來的拼法、帳本那側寫入時就已 canonical,於是賣掉的部位其出場敘事歸在
    `AAA#…`、thesis 卻掛在 `aaa#…`,永遠不會被結案。
...
def current_cycles(rows):
    ...
    """
    seq, start, _add_count = _cycle_walk(rows)
...
def current_cycle_add_cursors(rows):
    ...
    """
    seq, start, add_count = _cycle_walk(rows)
    out = {}
