...
- **卡片結構**(文字 / HTML 同):總覽(含市場背景一行,有數據才出)→ 做對的 → 標的層 → 最大的洞(數字 / 實例 / 動機 / 萬一)→ 報酬歸因 → 下次只改 + 引言。
...
〔市場那週〕{SPY 週{±%} · QQQ 週{±%} · VIX 收 {X}(前週 {Y})}          ← #37 語境一行:market_context 有數據才出,離線整行不出、不硬掰;
