...
The HTML card renders at the visual level of the `card-template.html` design reference: flat, light and dark via `prefers-color-scheme`, system font stack, a single heading, outlined tags, neutral section surfaces with semantic color only on section labels and P&L accents, no emoji, no icon font, and zero external requests. The rich layout blocks come from the same structured facts both surfaces share (#247): a KPI tile grid for the headline figures, ranked per-instrument money bars with engine behavior tags, the concentration stress row, benchmark-comparison attribution bars, and prescription rows — each block renders only when its engine fields exist and degrades silently otherwise, and engine zh vocabulary (tags, stress, prescriptions) stays off the English card until the engine grows localized equivalents. The document wraps a host-independent widget fragment between `<!-- WIDGET-FRAGMENT-START -->` and `<!-- WIDGET-FRAGMENT-END -->` markers for graphical surfaces. When `pnl_curve.points` carries at least two points, the HTML adds a small inline-SVG P&L sparkline colored by the final sign; note-form or missing curve data omits the sparkline silently and must not create a new user-facing caveat unless the honesty ledger requires one.
