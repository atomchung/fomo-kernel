...
Create an inferred thesis for every open cycle so later evidence can extend its history. State clearly that this is an opening portfolio check and invite the user to provide transaction history later to unlock supported behavioral diagnostics — name the concrete route (a broker CSV export, passed to the trade command `AGENTS.md` step 3 already shows: `prepare <CSV...>`) rather than leaving "provide it later" as an instruction with no stated next action; `copy/*.json`'s `block_missing.snapshot_unlock` carries this same invitation onto the card itself. Later transactions do not assert a fresh broker view: ledger-derived current holdings remain canonical, and only the explicit snapshot reconciliation below may certify or adopt a newer broker view.
...
A holdings view records the book at the time it arrives whatever it covers, so never ask whether it covers the user's whole account — that is an external account this product does not model, and there is no engine rejection behind the question. A declaration older than the current anchor is still rejected.
