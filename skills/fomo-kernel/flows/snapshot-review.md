...
When the coach root already has an accounting anchor and the user is handing over a newer view to *update their book* rather than to be reviewed, that is `flows/book-refresh.md`, not this flow — it asks about anything that disappeared, adopts the rest without ceremony, and costs no review. Stay here only when the user actually wants the review that comes with the declaration.
