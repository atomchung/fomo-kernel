...
3. Ask only items in `question_queue`. Prepare already deduplicated them against active theses and add counts; do not ask raw engine `thesis_questions` again. A stem may already quote the user's own recorded thesis and why the question was picked; present it as-is and do not re-verify or re-ask the quoted thesis.
