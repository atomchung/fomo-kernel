...
- every other position keeps the start the record already held for it, with the stamp that says what that date is worth (#539). Nothing to report — it is the absence of a change, and the point is that the user's earlier answers and theses stay attached to the same position rather than being asked for again. If you do report one, the stamp is what decides how: `trade_event` is exact, because the ledger watched that cycle open, and may be stated as a day; `snapshot_anchor` is the day a declaration first put the position on the books, so it is a lower bound — "on the books since at least March" — and never the day it was bought; `user_estimate` stays the approximation it always was.
