...
Headless `claude -p` does not expose the same interactive question tool as a normal session, so it can exercise only the text fallback. Artifact checkers remain valid because they inspect outputs rather than the internal conversation. Tool-order adherence and interactive option behavior require an interactive session or real user feedback.
