...
Use this owner-only gate for Claude, Codex, or another interactive host. It tests the experience a headless agent cannot prove. Keep real trade files, Review Plans, receipts, cards, and state roots outside the repository.
...
2. Start one `/tmp` UX receipt per host. Declare `plain_text` and `markdown_inline` unconditionally; add `native_options` and `widget` only if the host exposes them.
...
7. Append `owner_verdict` with controls and card set from the owner's direct experience, then run receipt verification with `--require-owner-verdict`.
...
2. Declare `prior_commitment` or `prior_skip` as expected memory. Also declare `exit_reason` and `due_revisit` when those fields are present in the Review Plan.
...
- Keep the raw failed receipt local.
