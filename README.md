...
- **資料流一句話自證**:本機 CSV → 你自己的 model session(Claude Code,或任何跑這個 skill 的 agent)→ 本機 `~/.trade-coach/`。**沒有第四站**——不落地任何第三方伺服器、不寫進任何雲端 DB,作者拿不到一行交易明細(這才是跟「會存檔、你看不到、作者能撈」的 SaaS 復盤工具的真差異)。
- **推理經過你自己的 model API**:agent 要讀 CSV 才能幫你復盤,交易內容會作為 context 經過你帳號用的 model API(用 Claude 就是 Anthropic)——跟你把對帳單貼給任何 AI 一樣,適用你自己帳號的資料政策。所以精確說不是「完全不經過任何伺服器」,而是「**不落地、不回作者、不進第三方**」。
- 唯一(自願)回收的是一句「這張卡有沒有用」,**不含交易內容**——願意給的話走 [card feedback 表單](https://github.com/atomchung/fomo-kernel/issues/new?template=card-feedback.yml),30 秒,這是 Stage 0 驗證「卡對別人有沒有用」的唯一來源。
